...
import { ArrowLeft, Sparkle, Trash2, Plus, GripVertical } from "lucide-react";
...
const SortableItem = ({
  ...
  return (
    <div ref={setNodeRef} style={style} className="group flex items-center bg-white rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300 relative overflow-hidden">
      <button className={`flex-1 flex items-center p-3 text-sm rounded-l-lg text-left transition-colors ${selectedChapterIndex === index 
        ? "bg-violet-50/5 text-violet-800 font-semibold"
      ...
      <div className="flex items-center ml-2 bg-white opacity-0 group-hover:opacity-100 transition-opacity px-2 py-3 absolute right-0">
        <Button  variant="ghost" size="small" className="py-2 px-2" onClick={() => onGenerateChapterContent(index)} isLoading={isGenerating === index} title="Generate content with AI">
          {isGenerating !== index && <Sparkle className="w-3.5 h-3.5 text-violet-800" />}
        </Button>
        <Button  variant="ghost" size="small" className="py-2 px-2" onClick={() => onDeleteChapter(index)} title="Delete Chapter">
          {isGenerating !== index && <Trash2 className="w-3.5 h-3.5 text-red-800" />}
        </Button>
...
const ChapterSidebar = ({ 
  ...
  const handleDragEnd = (event) => {
    ...
    if (active.id !== over.id) {
      ...
      const newIndex = chapterIds.indexOf(active.id);
