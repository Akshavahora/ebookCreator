  ...
  return <header>
    <div className="max-w-7xl mx-auto px-6 lg:px-8 ">
      <div className="flex items-center justify-between h-16">
        ...
        <nav className="hidden lg:flex items-center space-x-1">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className="px-4 py-2 text-sm font-medium text-gray-600 hover:text-violet-600 rounded-lg hover:bg-violet-50/5 transition-all duration-200"
            >
              {link.name}
            </a>
          ))}
        ...
        <div className="hidden lg:flex items-center space-x-3">
          {isAuthenticated ? (
            ...
            <>
              <a
                href="/login"
                className="px-4 py-2 text-sm font-medium text-shadow-gray-600 hover:text-shadow-gray-900 rounded-lg hover:bg-gray-50 transition-all duration-200"
              >
                Login
              </a>
              <a
        ...
        >
          {isOpen ? <X className="h-5 w-5" /> : <Menu className="w-5 h-5" />}
        </button>
      </div>
    </div>
    ...
    {/* Mobile Menu */}
    {isOpen && (
      <div className="lg:hidden bg-white border-t border-gray-100 animate-in slider-in-from-top duration-200">
        <nav className="px-4 py-4 space-y-1">
          {navLinks.map((link) => (
            <a
              key={link.name}
              href={link.href}
              className="block px-4 py-2.5 rounded-lg font-medium text-gray-700 hover:text-violet-600 hover:bg-violet-50 transition-all duration-200"
            > 
              {link.name}
            </a>
          ))}
          ...
          {isAuthenticated ? (
            <div className="space-y-3">
              <div className="flex items-center space-x-3 px-2">
                ...
                <div className="text-sxs text-gray-500">
                  {user?.email}
                </div>
              </div>
              <button
                className="w-full px-4 py-2.5 text-sm font-medium text-red-600 hover:bg-reg-50 rounded-lg transition-all duration-200 flex items-center justify-center space-x-2"
                onClick={() => LogOut()}
              >
