...
const exportAsDocument = async (req, res) => {
    ...
    book.chapter.forEach((chapter, index) => {
            try {
                ...
                sections.push(
                    new Paragraph({
                        ...
                        spacing: {
                            before:
                                DOCX_STYLES.spacing.chapterBefore,
                            after:
                                DOCX_STYLES.spacing.chapterAfter,
                        },
                    })
                );
        ...
        res.setHeader(
            "Content-Disposition",
            `attachment; filename=${book.title.replace(
                /[^a-zA-Z0-9]/g,
                "_"
            )}.docx`
        );
